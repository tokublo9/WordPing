  ...
  en: [
    {
      ...
      information: 'Apple processes purchase, subscription, device, and account information under its own terms and privacy policy. WordCore does not receive complete payment-card details.',
    ...
    {
      name: 'Cloudflare',
      purpose: 'Hosting and protecting the WordCore API Worker, storing short-lived entitlement and abuse-prevention counters, and caching shared promotional voice clips.',
      information: 'Network information such as IP address, request metadata, pseudonymous salted hashes, request counters, entitlement tier cache entries, and operational logs may be processed. User text and authorization credentials are excluded from WordCore application logs.',
      policyUrl: 'https://www.cloudflare.com/privacypolicy/',
    },
    {
      name: 'OpenAI',
      purpose: 'Generating High-Quality AI Voice audio and any other server-assisted AI output that WordCore makes available.',
      information: 'The text submitted for generation and generation settings such as voice or language are sent through the WordCore API Worker. Pseudonymous WordCore identifiers are not intentionally included in the prompt sent to OpenAI.',
  ...
  ja: [
    {
      ...
      information: 'Appleは、同社の利用規約およびプライバシーポリシーに基づき、購入、サブスクリプション、端末およびアカウントに関する情報を取り扱います。WordCoreがクレジットカード番号等の完全な決済情報を取得することはありません。',
    ...
    {
      name: 'Cloudflare',
      purpose: 'WordCore API Workerのホスティングおよび保護、短期間の利用資格・不正利用防止カウンターの保存、ならびに共通のプロモーション音声のキャッシュ。',
      information: 'IPアドレス等のネットワーク情報、リクエストのメタデータ、ソルト付きハッシュによる仮名識別子、利用回数、利用資格の一時キャッシュおよび運用ログが取り扱われる場合があります。利用者が入力したテキストおよび認証情報は、WordCoreのアプリケーションログには記録しない設計です。',
      policyUrl: 'https://www.cloudflare.com/ja-jp/privacypolicy/',
    },
    {
      name: 'OpenAI',
      purpose: '高品質AI音声、およびWordCoreが提供するその他のサーバー経由AI出力の生成。',
      information: '生成対象として送信されたテキストと、音声・言語等の生成設定がWordCore API Workerを経由して送信されます。WordCoreの仮名識別子をOpenAIへのプロンプトに意図的に含めることはありません。',
...
export const privacyDocuments: Record<LegalLocale, LegalDocument> = {
  en: {
    ...
    description: 'How WordCore handles information in the iOS app and on the WordCore website.',
    effectiveDateLabel: 'Effective date',
    effectiveDate: LEGAL_EFFECTIVE_DATE_EN,
    introduction: [
      `This Privacy Policy explains how ${LEGAL_OPERATOR} (the “Operator,” “we,” “us,” or “our”) handles information in connection with the WordCore iOS application, the WordCore website, and related API services (collectively, “WordCore”).`,
    ...
    sections: [
      ...
      {
        heading: '5. Analytics and session recording',
        paragraphs: [
      ...
      {
        heading: '6. Backup and data transfer',
        paragraphs: [
      ...
      {
        heading: '8. Third-party services and disclosure',
        paragraphs: [
  ...
  ja: {
    title: 'プライバシーポリシー',
    description: 'WordCore iOSアプリおよび公式ウェブサイトにおける情報の取扱いについて説明します。',
    effectiveDateLabel: '施行日',
    effectiveDate: LEGAL_EFFECTIVE_DATE_JA,
    introduction: [
      `本プライバシーポリシーは、${LEGAL_OPERATOR}（以下「運営者」または「当方」といいます）が提供するWordCore iOSアプリ、WordCore公式ウェブサイトおよび関連APIサービス（総称して「WordCore」といいます）における情報の取扱いを定めるものです。`,
      'WordCoreは、端末内での保存を基本とする単語学習アプリです。学習内容の大部分は利用者の端末内に保存されます。外部サービスへの情報送信は、利用者が実行した機能、サブスクリプションの確認、セキュリティ確保、またはウェブサイト配信に必要な場合に限って行われます。',
    ],
    sections: [
      {
        ...
        bullets: [
          ...
          'オンボーディングの回答（利用目的、学習する言語と説明の言語）。端末内にとどまり、外部へ送信されることはありません。第5項を参照してください。',
          'AIデータ共有を許可したかどうか。',
      ...
      },
      {
        ...
        paragraphs: [
          'WordCoreのAI機能は、OpenAIを利用して提供されます。AI機能を利用すると、リクエストは端末からCloudflare上で稼働するWordCore API Workerへ送信され、Workerが生成リクエストをOpenAIのAPIへ転送します。WordCoreは、これ以外のAI事業者を利用していません。',
          'WordCoreは、最初の当該リクエストの前にアプリ内で許可を求め、許可が与えられない限りOpenAIへ何も送信しません。この許可は独立した明示的な選択であり、利用規約への同意、オンボーディングの完了、サブスクリプションの購入によって与えられることはなく、アプリの更新によって既存の利用者に自動的に付与されることもありません。回答せずにダイアログを閉じた場合、データは送信されず、次にAI機能を利用する際にあらためて確認します。',
        ],
        bullets: [
          ...
          'WordCore API Workerが追加で受け取る情報：端末のキーチェーンに保存されたランダムなインストール識別子、仮名化されたRevenueCat App User ID、および音声を生成するカードのID。サブスクリプション区分の確認、不正利用防止およびカード枚数枠の適用に使用され、OpenAIへ転送されることはありません。',
          'Workerにおける取扱い：RevenueCatで利用資格を確認し、レート制限および利用上限を適用したうえで、生成リクエストをOpenAIへ渡します。生成された音声はWorkerに保存されることなく端末へ送出されます。送信されたテキストは、Workerのアプリケーションログおよびキーバリューストアには記録されません。ログに記録されるのは、エンドポイント、ステータス、リクエストID、処理時間、入力文字数、選択された音声および形式等、内容を含まない値のみです。Cloudflareは、サービスの提供および保護のため、IPアドレスを含むネットワーク情報を取り扱います。WordCoreは、IPアドレスをレート制限用のソルト付きハッシュの生成にのみ使用し、記録または保存しません。',
          '音声選択画面のプレビューはサブスクリプション向けの機能であり、読み上げる文がWordCoreの用意した固定テキストである場合も、他のAIリクエストと同様に利用者の許可の対象となります。',
          'アップグレード画面のプロモーション音声のみが例外であり、サブスクリプションおよび許可なしで再生できます。そのリクエストにはテキスト欄も音声欄も存在せず、アプリが送信するのは、WordCoreが用意した2種類の固定サンプルのいずれを再生するかという指定、固定の翻訳一覧から1つを選ぶための言語コード、および共通キャッシュを更新するためのビルド版数のみです。利用者の単語データは含まれず、前述の識別子（インストール識別子およびRevenueCat App User ID）も送信されません。プロモーション音声の再生のためにインストール識別子を新たに作成することもありません。これらの音声は一度生成された後、全利用者共通のものとしてWorkerにキャッシュされるため、通常はOpenAIではなくキャッシュから配信されます。再生してもAIデータ共有の許可が与えられることはなく、他のAI機能が有効になることもありません。',
      ...
      {
        heading: '5. 分析および画面の記録',
        paragraphs: [
          'WordCoreは、プロダクト分析を取得せず、画面の記録も行いません。アプリには分析SDKが組み込まれておらず、セッションの記録、仮名の分析用識別子、および利用状況の送信先となる分析事業者のいずれも存在しません。',
          'したがって、アプリ内に利用状況の共有を切り替える設定はありません。無効化の対象となるものが存在しないためです。',
        ],
        bullets: [
          'オンボーディングの回答（利用目的、学習する言語と説明の言語）は端末内に保存され、アプリの初期設定に使用されます。当方その他の第三者へ送信されることはありません。年齢、性別、WordCoreを知ったきっかけは質問しません。',
          'アプリ内の操作が外部へ報告されることはありません。単語の作成・削除、フォルダ名の変更、テストの回答、バックアップの書き出し・読み込みは、すべて端末内で完結します。',
          'クラッシュ・エラー情報を当方が取得することはありません。利用者自身の端末の分析設定に基づきAppleが取得・共有する情報については、本ポリシーではなくAppleの定めが適用されます。',
          'WordCoreは広告識別子（IDFA）を使用せず、Appのトラッキングの透明性（ATT）の許可を求めることもなく、他社のアプリやウェブサイトをまたいで利用者を追跡することもありません。',
          '第2項に記載のオンライン機能（AI生成およびサブスクリプションの確認）は、本項による変更を受けません。これらは同項に記載の内容を送信するものであり、分析には該当しません。',
        ],
      },
      {
        heading: '6. バックアップおよびデータ移行',
        paragraphs: [
      ...
      },
      {
        ...
        bullets: [
          ...
          '有料利用資格の確認、BasicのAI音声対象カード10枚の枠の適用、および購入の復元を行うため。',
          '不正利用の防止、リクエスト・利用上限の適用、障害調査、サービス可用性の保護、および運用費用の管理のため。',
          '問い合わせへの対応および法的義務の履行のため。',
          'WordCoreアプリおよびウェブサイトの運営、保護および改善のため。',
        ],
      },
      {
        heading: '8. 外部サービスおよび第三者提供',
        paragraphs: [
          ...
          'WordCoreは個人情報を販売せず、第三者広告SDKも使用していません。また、第三者のプロダクト分析SDKも一切使用していません（第5項を参照）。',
        ],
      },
      {
        heading: '9. 保存期間',
        bullets: [
          ...
          'BasicのAI音声対象カード枠には、RevenueCat App User IDおよびカードIDのソルト付きハッシュを使用します。カードごとの利用記録は月末に失効せず、更新や再インストールで10枚分の枠が再付与されないよう保持されます。失敗した生成の予約は解除または期限切れとなり、カード枠を消費しません。',
          '共通の音声プレビューおよびプロモーション音声キャッシュは、約30日保存される場合があります。これらはWordCoreが固定した共通テキストであり、利用者が入力したテキストではありません。',
          '保存対象となる分析データおよび画面の記録は存在せず、これらを保持する分析事業者もありません。',
          'オンボーディングの回答は端末内に保存され、アプリの削除とともに消去されます。回答から何らかの情報が導出され、外部へ送信されることはありません。',
          '運用ログおよびサポート連絡は、セキュリティ、障害調査、サポート、法令遵守および紛争対応のため合理的に必要な期間に限り、各事業者の設定および法的義務に従って保存されます。',
        ],
      },
      {
        heading: '10. ウェブサイトの保存機能',
        paragraphs: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'WordCoreはプロダクト分析を取得せず、画面の記録も行わないため、無効化の対象となる利用状況共有の設定は存在しません。第5項を参照してください。',
...
export const termsDocuments: Record<LegalLocale, LegalDocument> = {
  en: {
    ...
    description: 'Terms governing use of the WordCore iOS application, website, subscriptions, and AI services.',
    effectiveDateLabel: 'Effective date',
    effectiveDate: LEGAL_EFFECTIVE_DATE_EN,
    introduction: [
      `These Terms of Service (“Terms”) form an agreement between you and ${LEGAL_OPERATOR} (the “Operator,” “we,” “us,” or “our”) concerning your use of the WordCore iOS application, website, and related services (collectively, “WordCore”).`,
      'By downloading, accessing, purchasing, or using WordCore, you agree to these Terms. If you do not agree, do not use WordCore.',
    ],
    sections: [
      {
        heading: '1. Eligibility and authority',
        paragraphs: [
          'You must have the legal capacity to agree to these Terms. If applicable law requires consent from a parent, guardian, or other authorized person, you may use WordCore only with that consent. If you use WordCore for an organization, you represent that you have authority to bind that organization.',
        ],
      },
      {
        heading: '2. WordCore service',
        paragraphs: [
          'WordCore provides local vocabulary storage, study cards, review and visibility tools, local reminders, optional data transfer, subscription features, and certain AI-assisted audio or text functions. Features may vary by version, plan, device, region, language, and service availability.',
          'WordCore is a study aid. It does not guarantee learning results, accuracy, completeness, pronunciation, translation quality, or suitability for any academic, professional, medical, legal, financial, safety-critical, or other particular purpose.',
        ],
      },
      {
        heading: '3. Licence to use WordCore',
        paragraphs: [
          'Subject to these Terms and the applicable Apple terms, the Operator grants you a limited, personal, non-exclusive, non-transferable, non-sublicensable, revocable licence to use WordCore on Apple devices that you own or control for lawful personal use. No ownership interest in WordCore is transferred to you.',
      ...
      },
      {
        ...
        paragraphs: [
          'You retain rights you have in vocabulary, notes, audio, and other content that you enter into WordCore (“User Content”). You grant the Operator and necessary service providers a limited right to process User Content only to provide features you request, secure and operate the service, and comply with law.',
      ...
      },
      {
        ...
        bullets: [
          ...
          'You can manage or cancel a subscription in your Apple account settings. Deleting WordCore does not cancel a subscription.',
          ...
          'WordCore uses RevenueCat to verify subscription entitlements. A temporary verification outage may delay access or purchase restoration.',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Basic: High-Quality AI Voice for the fronts of up to 10 cards, selected from the top of the first folder and then down each Word List in folder order. Other card fronts and all backs use the free device voice. The 10-card allowance is granted once and does not reset monthly.',
          'Premium: High-Quality AI Voice for card fronts. AI Voice for backs can be enabled in Settings. Reasonable per-request, per-minute, per-day, character, safety, technical, and abuse-prevention limits continue to apply.',
          'Playing an existing cached clip or regenerating audio for a card already in the Basic allowance does not claim another card. Promotional and voice-picker samples do not claim a card. A failed generation does not claim a new card.',
      ...
      },
      {
        ...
        bullets: [
          'Use WordCore unlawfully, fraudulently, to infringe intellectual-property or privacy rights, or to generate or distribute harmful or illegal content.',
          'Circumvent subscriptions, quotas, rate limits, entitlement checks, security controls, or technical restrictions.',
          'Probe, disrupt, overload, scrape, reverse engineer, or attempt unauthorized access to WordCore or its providers, except where applicable law expressly permits an activity that cannot be restricted by contract.',
      ...
      },
      {
        ...
        paragraphs: [
          'WordCore, including its software, design, branding, text, graphics, and original media, is owned by the Operator or its licensors and is protected by applicable law. These Terms do not grant permission to use the WordCore name, logo, or proprietary assets outside normal use of the service.',
          'WordCore includes third-party open-source and source-available components governed by their own licence terms. Those terms control for the applicable component. Notices are available on the Open Source Licences page.',
        ],
      },
      {
        heading: '10. Third-party services',
        paragraphs: [
          'WordCore relies on Apple, RevenueCat, Cloudflare, OpenAI, Vercel, and open-source software. Third-party services may be changed, interrupted, restricted, or discontinued independently of the Operator. Your use of those services may also be subject to their terms and privacy policies.',
        ],
      },
      {
      ...
      },
      {
        ...
        paragraphs: [
          'We may update these Terms for changes to WordCore, providers, law, security, or business practices. Updated Terms will be posted with a revised effective date. Where required by law, we will provide additional notice or obtain consent. Continuing to use WordCore after an update takes effect constitutes acceptance only to the extent permitted by applicable law.',
        ],
      },
      {
        heading: '15. Governing law and jurisdiction',
        paragraphs: [
          'These Terms are governed by the laws of Japan, without regard to conflict-of-law rules. The Kumamoto District Court shall be the agreed court of first instance for disputes arising from or relating to WordCore or these Terms, subject to any mandatory consumer-protection laws or jurisdiction rules that apply.',
        ],
      },
      {
        heading: '16. General provisions',
        paragraphs: [
          'If a provision is held invalid or unenforceable, it will be enforced to the maximum lawful extent and the remaining provisions will continue in effect. Failure to enforce a provision is not a waiver. You may not assign these Terms without the Operator’s consent; the Operator may assign them as part of a lawful transfer of the service, subject to applicable law. These Terms, the Privacy Policy, applicable Apple terms, and any purchase terms shown at checkout constitute the agreement concerning WordCore.',
        ],
      },
      {
        heading: '17. Contact',
  ...
  ja: {
    title: '利用規約',
    description: 'WordCore iOSアプリ、ウェブサイト、サブスクリプションおよびAIサービスの利用条件です。',
    effectiveDateLabel: '施行日',
    effectiveDate: LEGAL_EFFECTIVE_DATE_JA,
    introduction: [
      `本利用規約（以下「本規約」といいます）は、利用者と${LEGAL_OPERATOR}（以下「運営者」または「当方」といいます）との間で、WordCore iOSアプリ、ウェブサイトおよび関連サービス（総称して「WordCore」といいます）の利用条件を定めるものです。`,
      'WordCoreをダウンロード、閲覧、購入または利用することにより、利用者は本規約に同意したものとみなされます。同意しない場合は、WordCoreを利用しないでください。',
    ],
    sections: [
      {
        heading: '1. 利用資格および権限',
        paragraphs: [
          '利用者は、本規約に同意するために必要な法的能力を有していなければなりません。適用法令上、親権者、保護者その他の権限を有する者の同意が必要な場合、その同意を得た場合に限りWordCoreを利用できます。組織のためにWordCoreを利用する場合、利用者は当該組織を本規約に拘束する権限を有することを表明するものとします。',
        ],
      },
      {
        heading: '2. WordCoreの内容',
        paragraphs: [
          'WordCoreは、端末内の単語保存、学習カード、復習・表示管理、ローカル通知、任意のデータ移行、サブスクリプション機能、および一部のAI音声・AIテキスト機能を提供します。機能は、バージョン、プラン、端末、地域、言語およびサービス提供状況により異なる場合があります。',
          'WordCoreは学習補助ツールです。学習成果、正確性、完全性、発音、翻訳品質、ならびに学術、業務、医療、法律、金融、安全上重要な用途その他特定目的への適合性を保証するものではありません。',
        ],
      },
      {
        heading: '3. 利用許諾',
        paragraphs: [
          '本規約および適用されるAppleの条件に従うことを条件として、運営者は、利用者が所有または管理するApple端末上で、適法な個人利用のためにWordCoreを使用する、限定的、個人的、非独占的、譲渡不能、再許諾不能かつ取消可能な権利を許諾します。WordCoreの所有権が利用者へ移転することはありません。',
      ...
      },
      {
        ...
        paragraphs: [
          '利用者がWordCoreへ入力した単語、メモ、音声その他の情報（以下「利用者コンテンツ」といいます）について、利用者が有する権利は利用者に留保されます。利用者は、要求した機能の提供、サービスの安全な運営および法令遵守に必要な範囲に限り、運営者および必要な外部事業者が利用者コンテンツを取り扱うことを許諾します。',
          ...
          'WordCoreは端末内保存を基本としており、単語データを自動的にサーバーへバックアップしません。Premium利用者はバックアップファイルを作成し、保存先または共有先を選択できます。利用者は、適切なバックアップの維持および書き出したファイルの保護について責任を負います。法令上免責できない場合を除き、端末の紛失、削除、OSの動作、移行の失敗、ファイル破損または利用可能なバックアップを維持しなかったことによるデータ損失について、運営者は責任を負いません。',
        ],
      },
      {
        ...
        bullets: [
          ...
          'サブスクリプションはAppleアカウントの設定から管理または解約できます。WordCoreを削除してもサブスクリプションは解約されません。',
          '請求、決済処理、返金の可否およびApp Store上の取引紛争は、Appleの条件および適用法令に基づきAppleが取り扱います。運営者がApp Storeの返金を直接行うことはできません。',
          'WordCoreは、サブスクリプション利用資格の確認にRevenueCatを利用します。確認サービスの一時的な障害により、利用資格の反映または購入復元が遅れる場合があります。',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Basic：追加した順に古いカードから最大10枚分の表面に高品質AI音声を生成します。それ以外の表面とすべての裏面は無料の端末音声です。この10枚分の枠は一度だけ付与され、毎月更新されません。',
          'Premium：カード表面に高品質AI音声を生成します。裏面のAI音声は設定で有効にできます。ただし、1回、1分、1日、文字数、安全性、技術上および不正利用防止上の合理的な制限は引き続き適用されます。',
          '端末内キャッシュからの再生や、Basicの対象カードの音声の再生成は、新しいカード枠を消費しません。プロモーションサンプルおよび音声選択画面のサンプルもカード枠を消費しません。生成に失敗した場合、新しいカード枠は消費されません。',
      ...
      },
      {
        ...
        bullets: [
          '違法または詐欺的な目的、知的財産権・プライバシー権の侵害、または有害・違法なコンテンツの生成・配布のためにWordCoreを利用する行為。',
          'サブスクリプション、利用枠、レート制限、利用資格確認、セキュリティ制御または技術的制限を回避する行為。',
          '適用法令が契約で制限できない行為として明示的に認める場合を除き、WordCoreまたは外部事業者への調査、妨害、過負荷、スクレイピング、リバースエンジニアリングまたは不正アクセスを試みる行為。',
      ...
      },
      {
        ...
        paragraphs: [
          'WordCoreのソフトウェア、デザイン、ブランド、文章、画像および独自メディアは、運営者または正当な権利者に帰属し、適用法令により保護されます。本規約は、通常のサービス利用を超えてWordCoreの名称、ロゴまたは独自素材を利用する許可を与えるものではありません。',
          'WordCoreには、それぞれのライセンス条件が適用される第三者のオープンソースまたはソース利用可能なコンポーネントが含まれます。当該コンポーネントについては各ライセンス条件が優先します。表示事項は「オープンソースライセンス」ページで確認できます。',
        ],
      },
      {
        heading: '10. 外部サービス',
        paragraphs: [
          'WordCoreは、Apple、RevenueCat、Cloudflare、OpenAI、Vercelおよびオープンソースソフトウェアを利用します。外部サービスは、運営者とは独立して変更、中断、制限または終了される場合があります。各外部サービスの利用には、当該事業者の利用規約およびプライバシーポリシーが適用される場合があります。',
        ],
      },
      {
      ...
      },
      {
        heading: '12. 保証の否認',
        paragraphs: [
          '法令で認められる最大限の範囲で、WordCoreは「現状有姿」かつ「提供可能な範囲」で提供されます。運営者は、商品性、特定目的適合性、権利非侵害、正確性、可用性、および中断・エラーのない動作に関する黙示の保証を否認します。本規約は、法令上排除できない保証または消費者の権利を排除するものではありません。',
        ],
      },
      {
        heading: '13. 責任の制限',
        paragraphs: [
          '適用法令で認められる最大限の範囲で、運営者は、WordCore、その利用不能、または生成結果への依拠に起因または関連する間接損害、付随的損害、特別損害、結果的損害、懲罰的損害、ならびにデータ、利益、売上、機会、信用の喪失または事業中断について責任を負いません。',
      ...
      },
      {
        heading: '14. 本規約の変更',
        paragraphs: [
          'WordCore、外部事業者、法令、セキュリティまたは事業運営の変更に応じて、本規約を変更する場合があります。変更後の規約は、改定後の施行日とともに掲載します。法令上必要な場合は、追加の通知または同意取得を行います。変更後もWordCoreを利用したことによる同意は、適用法令で認められる範囲に限り効力を有します。',
        ],
      },
      {
        heading: '15. 準拠法および合意管轄',
        paragraphs: [
          '本規約は、法の抵触に関する規則を除き、日本法に準拠します。WordCoreまたは本規約に起因または関連する紛争については、適用される強行的な消費者保護法または管轄規則に従うことを条件として、熊本地方裁判所を第一審の合意管轄裁判所とします。',
        ],
      },
      {
        heading: '16. 一般条項',
        paragraphs: [
          '本規約の一部が無効または執行不能と判断された場合、当該条項は適法な最大限の範囲で適用され、その他の条項は引き続き有効とします。権利を行使しないことは放棄を意味しません。利用者は運営者の同意なく本規約上の地位を譲渡できません。運営者は、適用法令に従い、サービスの適法な承継に伴い本規約を譲渡できます。本規約、プライバシーポリシー、適用されるAppleの条件および購入時に表示される条件は、WordCoreに関する合意を構成します。',
        ],
      },
      {
        heading: '17. お問い合わせ先',
