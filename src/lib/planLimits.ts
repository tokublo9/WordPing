 ...
 *
 * Premium has a 400-generation monthly budget. The Worker also enforces
 * 200 generations per UTC day for that tier.
 *
 * `0` here does not mean "no access". Basic sits at zero because it has no
 * *monthly* allowance — its access is the lifetime card grant in
 ...
 *
 * Card fronts claim Basic slots. Re-generating a claimed card, cached audio,
 * voice-picker previews and device TTS do not claim another slot.
 */
...
/**
 * Lifetime High-Quality AI Voice card slots, mirroring the Worker's table.
 *
 * `null` means no one-time card grant (Premium). `0` means the plan does not have the
 * feature (Free). Basic's number is granted once per subscriber and never
 ...
 *
 * Returns null for Premium so the table can render its usual "included" circle,
 * and null for a plan that does not have it
 ...
 *
 * A one-time grant is labelled as one. Calling Basic's 10 a monthly figure
...
export function formatVoiceMonthlyLimit(
  ...
  basicCardLimit = VOICE_LIFETIME_CREDITS.basic ?? 0,
): string | null {
  ...
  // The comparison table shows inclusion with a circle. The request budget is
  // enforced by the Worker and is not a per-card feature allowance.
  if (tier === 'premium') return null;
  if (monthly !== null && monthly !== 0) {
    return t('cmp_voice_per_month').replace('{n}', format(monthly));
  }

  const lifetime = tier === 'basic' ? basicCardLimit : VOICE_LIFETIME_CREDITS[tier];
