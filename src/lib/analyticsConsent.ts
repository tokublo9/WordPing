// ── Retired: the product-analytics consent store ─────────────────────────────
//
// PostHog has been removed — see src/config/posthog.ts — so there is no
// analytics to consent to. The Share Usage Data row that wrote this store is
// gone from Settings, and nothing imports this module any more. It is kept,
// commented out, so restoring analytics restores a reviewed opt-out rather
// than shipping one written from scratch.
//
// Note for a future restore: the stored key is untouched, so a device that
// recorded a decision before the removal still carries it.
//
...
//  * the explanation language, the learning language and the learning purpose,
//  * which are all onboarding asks — published only on the enable path and never
//  * while this reads `disabled`.
