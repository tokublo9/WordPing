/**
 * PostHog is DISABLED. WordCore no longer sends product analytics.
 *
 * Nothing in the app constructs a client any more, so there is no event
 * capture, no Session Replay, no Person Properties and no analytics identifier.
 * `App.tsx` mounts no `PostHogProvider`, and every `posthog?.capture(...)` call
 * site is commented out beside its import rather than left as a silent no-op.
 *
 * The whole previous configuration is preserved below, commented out, so the
 * integration can be restored deliberately instead of rebuilt from memory.
 * Restoring it means uncommenting this file *and* the call sites in
 * `features/cards/useCards.ts`, `features/folders/useFolders.ts`,
 * `components/TestModeScreen.tsx` and `components/BackupSection.tsx`, putting
 * the Share Usage Data row back in Settings, and putting the analytics
 * sections back into the privacy policy — the policy now states that no
 * analytics are collected, and that statement must stay true.
 *
 * `posthog` is still exported, permanently `undefined`, so the type of every
 * call site stays honest without a second definition of "analytics are off".
 */

import type PostHog from 'posthog-react-native';

export const posthog: PostHog | undefined = undefined;

/**
 * Was: republish the onboarding research Person Properties when onboarding
 * completed. There is no analytics client, so there is nothing to publish and
 * nothing to read from storage.
 */
export function publishAnalyticsResearchProperties(): void {
  // Intentionally empty — see the note at the top of this file.
}

// ── Everything below is the retired PostHog integration ──────────────────────
